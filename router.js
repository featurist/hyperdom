...
exports.render = function(mount) {
  history.start()
  var url = history.url()
  var route

  if (lastUrl != url) {
    route = setUrl(url, mount.model.renderUrl())
    lastUrl = url
  } else {
    var newUrl = getUrl(url, mount.model.renderUrl())
    if (newUrl) {
      if (lastUrl != newUrl.url) {
        if (newUrl.push) {
          history.push(newUrl.url)
        } else {
          history.replace(newUrl.url)
        }
        lastUrl = history.url()
      }
      route = newUrl.route
...
function route(pattern) {
  ...
  route.push = function(params) {
    history.push(expand(patternVariables.pattern, params))
    lastMount.rerender()
  }

  route.replace = function(params) {
    history.replace(expand(patternVariables.pattern, params))
    lastMount.rerender()
  }
...
function setUrl(url, routes) {
  var match;
  var route = routes.find(r => match = r.match(url));
  if (route) {
    route.set(url, match);
    return route
  }
...
function getUrl(url, routes) {
  var route = routes.find(r => r.match(url));
  if (route) {
...
HrefAttribute.prototype.hook = function (element, propertyName) {
  var self = this

  element.href = this.route.url(this.params)
  element.onclick = function() {
    self.route.push(self.params)
    return false
  }
};

HrefAttribute.prototype.unhook = function (element, propertyName) {
  element.onclick = null
};

var history = exports.historyApi
